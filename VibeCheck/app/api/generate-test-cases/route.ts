...
async function getRepoTree({
  ...
  let finalToken = githubToken && githubToken !== "undefined" && githubToken !== "null" ? githubToken : fallbackToken;
  ...
  console.log(`[DEBUG] Fetching GitHub API: ${url}`);
  console.log(`[DEBUG] Using Token: ${finalToken ? finalToken.substring(0, 10) + '...' : 'None'}`);
  
  let res = await fetch(url, { headers });

  if (!res.ok && res.status === 401 && headers.Authorization) {
    delete headers.Authorization;
    if (fallbackToken && fallbackToken !== finalToken) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to fetch GitHub repo tree: ${res.status} ${res.statusText}`);
  }
